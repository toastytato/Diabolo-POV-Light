...
#define THETA         360 / NUM_SECTORS  //angle per sector
    ...
    volatile uint8_t displayLed[NUM_SECTORS][NUM_LEDS]; //matrix of led state in ram

    Adafruit_NeoPixel pixels = Adafruit_NeoPixel(NUM_LEDS, LED_PIN, NEO_GRB + NEO_KHZ800);

    uint8_t symbol_cursor = 0;
    uint8_t symbol_spacing = 1;
  ...
  public:
    ...
    void show_sector() {
      for (uint8_t i = 0; i < NUM_LEDS; i++) {
        uint8_t state = displayLed[currSector][NUM_LEDS - i]; //0 or 1
